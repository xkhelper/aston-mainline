...
static inline int syna_pal_completion_wait_for(syna_pal_completion_t *ptr, unsigned int timeout_ms) {
	int retval;

	retval = wait_for_completion_timeout((struct completion *)ptr, msecs_to_jiffies(timeout_ms));
	if (retval == 0)
		return -1;
...
static inline int syna_pal_str_cpy(char *dest, unsigned int dest_size, const char *src, unsigned int src_size, unsigned int num) {
	if (dest == NULL || src == NULL)
		return -1;
...
static inline int syna_pal_str_cmp(const char *str1, const char *str2, unsigned int num) {
...
static inline unsigned int syna_pal_crc32(unsigned int seed, const char *data, unsigned int len) {
